"""PDF Editor の「普通のソフト」用インストーラ (Setup.exe) を作る。
...
出力:
    %USERPROFILE%\\PDFEditor_release\\PDFEditor-Setup-<版>.exe
"""
...
ISS = os.path.join(HERE, "installer", "PDFEditor.iss")
...
def _source_dir() -> str:
    """onedir ビルド（PDFEditor.exe と依存一式）のフォルダを返す。"""
    return os.path.join(
        os.path.expanduser("~"), "PDFEditor_dist", "dist", "PDFEditor"
    )
...
def main() -> int:
    ...
    src = _source_dir()
    exe = os.path.join(src, "PDFEditor.exe")
    ...
    icon = os.path.join(os.path.expanduser("~"), "PDFEditor_dist", "_build", "pdfeditor.ico")
    ...
    setup = os.path.join(out_dir, f"PDFEditor-Setup-{version}.exe")
    print("\n=== 完了 ===")
    print("インストーラ:", setup)
